...
{
	if(all->map.map[(int)(all->start.pos.x + (all->start.dir.x * all->moveSpeed * i))][(int)all->start.pos.y] == 0) 
	 	all->start.pos.x += all->start.dir.x * all->moveSpeed * i;
	if(all->map.map[(int)(all->start.pos.x)][(int)(all->start.pos.y + (all->start.dir.y * all->moveSpeed * i))] == 0) 
