...
/*   Updated: 2020/03/16 12:33:50 by robriard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_fillcolor(char *line, int *color)
{
	int		i;
...
t_color		ft_color(char *line)
{
	...
	while (line[++i])
		if (!(line[i] >= '0' && line[i] <= '9') && line[i] != ' ')
			return (ret);
	i = 1;
	i += ft_fillcolor(line + i, &ret.red);
	i += ft_fillcolor(line + i, &ret.green);
	i += ft_fillcolor(line + i, &ret.blue);
	return (ret);
}
